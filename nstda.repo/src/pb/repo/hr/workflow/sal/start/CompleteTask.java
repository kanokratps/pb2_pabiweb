package pb.repo.hr.workflow.sal.start;
...
@Component("pb.hr.workflow.sal.start.CompleteTask")
public class CompleteTask implements ExecutionListener {
	...
	public void notify(final DelegateExecution execution) throws Exception {
		log.info("<- hr.start.CompleteTask ->");
...
//					finalAction = mainWorkflowService.saveWorkflowHistory(execution, curUser, taskKey, taskComment, finalAction, null,  model.getId(), level, HrSalConstant.ST_WAITING);
					execution.setVariable("LEVEL", 1);
				}
				catch (Exception ex) {
					log.error(ex);
				}
