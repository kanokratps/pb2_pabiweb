		...
		columns.push({ text: PBExpUse.Label.m.status,  dataIndex: 'wfstatus', width:370,
		  	  renderer: function (v, m, r) {
        			...
        			if (status != "D") {
			            ...
			            return Ext.String.format('<div><div style="float:left;margin:0;padding:0;width:12px;height:20px" class="icon_bar_{2}" id="{1}"></div><div id="{0}"></div></div>', id, id1, PBExpUse.Label.s[status].color);
                    } else {
			            var id = Ext.id();
			            return Ext.String.format('<div style="float:left;margin:0;padding:0;width:12px;height:20px" class="icon_bar_{1}" id="{0}"></div>', id, PBExpUse.Label.s[status].color);
                    }
		      }
		 });
...
//		var s = eval('me.indicator[r.get("status")].text_' +me.lang);
		var s = eval('PBExpUse.Label.s[r.get("status")].text'+r.get("pay_type")+'_' +me.lang);
//		console.log("s1:"+s+","+r.get("status")+","+r.get("pay_type"));
		if(!s) {
			s = eval('PBExpUse.Label.s[r.get("status")].text_' +me.lang);
		}
//		console.log("s2:"+s);
